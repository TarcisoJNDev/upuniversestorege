...
async function fetchRelatedProducts(categoryId, currentProductId) {
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.BASE_URL}/products/category-id/${categoryId}`,
    );
    if (!response.ok) {
      throw new Error(`Erro HTTP: ${response.status}`);
    }
...
function renderTabContent(product) {
  ...
  if (detailedDescription) {
    ...
    if (product.description) {
      ...
      descriptionHTML +=
        "<p>Não há descrição detalhada disponível para este produto.</p>";
    }
    ...
    if (
      ...
      descriptionHTML +=
        '<h4 style="margin-top: 20px;">Características:</h4><ul style="margin: 15px 0; padding-left: 20px; line-height: 1.8;">';
  ...
  try {
    ...
    relatedGrid.innerHTML = relatedProducts
      .map((relatedProduct) => {
        ...
        const relatedPrice =
          relatedProduct.promotional_price || relatedProduct.price;
...
function setupEventListeners(product) {
  ...
  // Botão de Comprar - MODIFICADO PARA USAR cartManager
  ...
  if (buyNowBtn) {
    buyNowBtn.addEventListener("click", async function () {
      ...
      // Usar cartManager.addToCart em vez da função antiga
...
// ============================================
// REMOVER a função addToCart antiga - não será mais usada
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    showError(
      "Não foi possível carregar as informações do produto. Tente novamente mais tarde.",
    );
  }
});
