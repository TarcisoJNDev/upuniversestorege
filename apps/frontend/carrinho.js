  ...
  for (const item of cart.items) {
    try {
      ...
      if (response.ok) {
        ...
        const maxQuantity = Math.min(stock, 10); // Limitar a 10 unidades
...
function setupEventListeners() {
  ...
  // Opção de frete
  // Opção de frete - CORRIGIDO
  const shippingSelect = document.getElementById("shipping-method");
  if (shippingSelect) {
    shippingSelect.addEventListener("change", function () {
      calculateTotalWithShipping(); // Recalcular quando mudar o frete
    });
  }
...
function updateCartItem(productId, quantity) {
  ...
  if (itemElement) {
    ...
    if (priceText.includes("text-decoration: line-through")) {
      // Tem promoção - pegar o preço promocional
...
function calculateTotalWithShipping() {
  ...
  // Atualizar interface
  subtotalElement.textContent = `R$ ${cart.total.toFixed(2)}`;
  totalElement.textContent = `R$ ${total.toFixed(2)}`;

  console.log(
    `📊 Cálculo: Subtotal R$ ${cart.total.toFixed(2)} + Frete R$ ${shipping.toFixed(2)} = Total R$ ${total.toFixed(2)}`,
  );
}
  ...
  if (itemElement) {
    ...
    setTimeout(() => {
      ...
      if (cart.items.length === 0) {
        ...
        calculateTotalWithShipping(); // Recalcular após remover
      }
    }, 300);
  }
}
